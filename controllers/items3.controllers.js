...
export const getItems = async (req, res) => {
    ...
    items.forEach((doc) => list.push({ id:doc.id, name: doc.data().name, price: doc.data().price }));
    res.json(list);
};
...
export const putItem = async (req, res) => {
    const item = await db.collection("items").doc(req.params.id).update(req.body);
    res.json({ id: req.params.id, name: req.body.name, price: req.body.price });
;};

export const deleteItem = async (req, res) => {
    const item = await db.collection("items").doc(req.params.id).delete();
